...
public class MergeBall : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //need to get that variable

        if(transform.localPosition.y > GameHandler.instance.deathPosY && isDropped)
        {
    ...
    public void DestructionEffect(float timer = 0.3f)
    {
        cannotCollide = true;
        mergeProcess = true;
        ...
        StopAllCoroutines();
        StartCoroutine(DestructionEffectProcess(timer));
    }
    IEnumerator DestructionEffectProcess(float timer = 0.3f)
    {
        //it disappears and then is destroyed.
        transform.DOScale(0, timer);
