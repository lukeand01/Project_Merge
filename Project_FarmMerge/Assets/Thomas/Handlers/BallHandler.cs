...
public class BallHandler : MonoBehaviour
{
    ...
    bool hasStarted;
    [SerializeField] ParticleSystem mergePS;
    ...
    public void OrderToStartBallHandler()
    {
        Invoke(nameof(StartBallHandler), 0.15f);
    }
    ...
    public void SpawnNewCurrentBall(bool isFirst = false)
    {
        ...
        newObject.transform.localPosition = new Vector3(newObject.transform.localPosition.x, newObject.transform.localPosition.y, -10);
    ...
    public void SpawnNewAtPosition(int index, MergeBall firstBall, MergeBall secondBall)
    {
        ...
        if(newObject.transform.localPosition.z != 0)
        {
            Debug.Log("htis fella");
        }

        GameObject newPS = Instantiate(mergePS.gameObject, firstBall.transform.position, Quaternion.identity);
        newPS.GetComponent<DestroySelf>().SetUpDestroy(1);
        ...
        scoreObject.transform.localPosition = new Vector3(scoreObject.transform.localPosition.x, scoreObject.transform.localPosition.y, -50);
    ...
    public void AwardPoint(MergeBall merge)
    {
        Color color = Utils.GetRandomColor();

        float score = GetMergeTypeScore(merge.mergeType);
        PlayerHandler.instance.AddPoints((int)score * (int)bonusValue);

        FadeUI scoreObject = Instantiate(fadeTemplate, merge.transform.position, Quaternion.identity);
        scoreObject.transform.SetParent(UIHandler.instance.transform);
        scoreObject.transform.localPosition = new Vector3(scoreObject.transform.localPosition.x, scoreObject.transform.localPosition.y, -50);
    ...
    public void DestroyedByFox(MergeBall mergeTarget)
    {
        //we destroy the target and give more bonus 
        Color color = Utils.GetRandomColor();

        AddBonus(10); //we give more stuff fro fox.
        float score = GetMergeTypeScore(mergeTarget.mergeType);
        PlayerHandler.instance.AddPoints((int)score * 2);

        FadeUI scoreObject = Instantiate(fadeTemplate, mergeTarget.transform.position, Quaternion.identity);
        scoreObject.transform.SetParent(UIHandler.instance.transform);
        scoreObject.transform.localPosition = new Vector3(scoreObject.transform.localPosition.x, scoreObject.transform.localPosition.y, -50);
    ...
    {
        float additionalModifier = 1;

        if((int)mergeType >= 5)
        {
            additionalModifier = 1.5f;
        }
        if ((int)mergeType >= 7)
        {
            additionalModifier = 2f;
        }
        if ((int)mergeType == 9)
        {
            additionalModifier = 3f;
        }

        return ((float)mergeType * (float)mergeType) + 1 * 1.5f * additionalModifier;
    }
    ...
    public void ReceiveMoveInput(Vector3 pos)
    {
        ...
        if (UIHandler.instance.confirmationUI.IsActive())
        {
            Debug.Log("confirm ui is open");
            return;
        }
    ...
    float GetTurnModifier(Vector3 mergePos, Color color)
    {
        ...
        if (mergeSpree > 1)
        {
            ...
            cheerObject.transform.localPosition = new Vector3(cheerObject.transform.localPosition.x, cheerObject.transform.localPosition.y, -50);
    ...
    float GetCurrentBonusGiftbasedInBonusAmount()
    {
        ...
        float clampedValue = 25 + (bonusAmount * 25);
